    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const athletes: Athlete[] = data.map((row, index) => {
          const rawEntry = row.entryHeight || row.entry_height || row['Entry Height'] || row['entry height'] || '';
          const parsedEntry = rawEntry ? parseFloat(String(rawEntry)) : undefined;
          return {
            ...
            status: 'active',
            consecutiveMisses: 0,
            ...(parsedEntry && parsedEntry > 0 ? { entryHeight: parsedEntry } : {}),
          };
        });
    ...
    <div className="p-6 border-2 border-dashed border-slate-300 rounded-2xl bg-white/50 text-center">
      <div className="flex flex-col items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Upload a CSV with name, school, bib, and optional entryHeight columns</p>
        </div>
        ...
        <div className="text-left w-full">
          ...
          <div className="bg-slate-100 p-3 rounded-lg font-mono text-xs text-slate-600">
            ...
            2,Jane Smith,Westside Academy,2.30
          </div>
        </div>
      </div>
    </div>
